...
Invoke-WebRequest -Uri "https://github.com/ipinfo/cli/releases/download/range2ip-$VSN/$FileName.zip" -OutFile ./$ZipFileName
Unblock-File ./$ZipFileName
Expand-Archive -Path ./$ZipFileName  -DestinationPath $env:LOCALAPPDATA\ipinfo -Force

# delete if already exists
if (Test-Path "$env:LOCALAPPDATA\ipinfo\range2ip.exe") {
    Remove-Item "$env:LOCALAPPDATA\ipinfo\range2ip.exe"
}
Rename-Item -Path "$env:LOCALAPPDATA\ipinfo\$FileName.exe" -NewName "range2ip.exe"
