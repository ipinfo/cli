...
// stripped.
func stripOverlapping(s []*ipUtils.CIDR) []*ipUtils.CIDR {
	l := len(s)
	for i := 0; i < l-1; i++ {
		if s[i] == nil {
			continue
...
func overlaps(a, b *ipUtils.CIDR) bool {
...
// combined.
func combineAdjacent(s []*ipUtils.CIDR) []*ipUtils.CIDR {
	for {
		found := false
		l := len(s)
		for i := 0; i < l-1; i++ {
